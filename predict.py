...
PTH_FILE = f"{DATA_DIR}/checkpoint_90.8831%.pth"  # 调用之前生成好的 pth 文件
...
def get_image():
    ...
    print("----> Image Resize To: ", train.IMAGE_SIZE)
    resize = transforms.Resize(size=train.IMAGE_SIZE, antialias=True)  # 调整图片尺寸
    img = resize(img)
    print("----> Image Shape: ", img.shape)
    print("----> Done")
